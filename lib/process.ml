...
let process_intro (sg : subgoal) (env : gam) (h : string option) =
  ...
  match goal with
  | Implication (t1, t2) ->
      let hyp_name = match h with None -> new_hyp () | Some n -> n in
      if hypothesis_name_exists hyp_name env hyps then raise HypothesisExists
      else
...
let process_apply (sg : subgoal) (h : string) =
  ...
  | Some (ty, vn) -> (
      ...
      | t :: tl ->
          if t <> goal then raise HypothesisMismatch
          else create_intermediate_subgoals hyps vn (List.rev tl) proof)
  ...
  | Some (t, vn) -> (
            ...
            Application
              ( ref
                  (Application
                     ( ref
                         (Abstraction
                            (vn1, t1, ref (Abstraction (vn2, t2, new_proof)))),
      ...
      | Disjunction (t1, t2) ->
          ...
          proof :=
            Case
              ( ref (Variable vn),
                ref (Abstraction (vn1, t1, left_proof)),
                ref (Abstraction (vn2, t2, right_proof)) );
          let hyps = List.remove_assoc h hyps in
          [
            (left_proof, goal, (h, (t1, vn1)) :: hyps);
            (right_proof, goal, (h, (t2, vn2)) :: hyps);
          ]
...
let process_absurd (sg : subgoal) (env : gam) (t : ty) =
  ...
  proof :=
    ExFalso
      ( ref
          (Application
             ( ref (Application (ref (Variable varname), proof_prop)),
               proof_neg_prop )),
        goal );
  let not_t = Implication (t, False) in
  ( [ (proof_prop, t, hyps); (proof_neg_prop, not_t, hyps) ],
    (varname, Implication (t, Implication (not_t, False))) :: env )
...
let print_current_proof (proof : proof ref) =
  (try print_lam (proof_to_term !proof)
   with IncompleteProof -> print_string "Proof not finished.");
  print_newline ()
...
let process_interactive_tactic (session : interactive_session) (t : tactic) :
  ...
  try
    ...
    | Qed -> (
        ...
        | Some proof -> (
            if sgs <> [] then StepError "Proof not finished."
            else
              let term = proof_to_term !proof in
              match goal with
              | None -> StepError "No goal."
              | Some goal ->
                  if Infer.typecheck env term goal then StepFinished term
                  else
