...
type lam =
  | Abstraction of string * ty * lam (* name of argument, type of argument and body *)
  | Application of lam * lam (* term we apply to, the term as argument *)
  | Variable of string (* name of variable *)
  | ExFalso of lam * ty (* term that should type to false, type we want from this *)
...
let rec alpha_aux (env : alpha_env) (t1 : lam) (t2 : lam) =
  ...
  | ExFalso (l1', _), ExFalso (l2', _) -> alpha_aux env l1' l2' (* We don't care about the type *)
...
let rec betastep (l : lam) =
  ...
  | ExFalso (b, t) -> (
    (* Reduce inside if possible *)
      match betastep b with None -> None | Some b' -> Some (ExFalso (b', t)))
  | Application (l1, l2) -> (
