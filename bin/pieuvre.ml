...
(* takes as input a lambda expression written in a file passed as an argument and displays the sequence of its reductions *)

let alpha = ref false
(* takes two lambda expressions as input, separated by the & character and written to a file passed as an argument, and returns true or false depending on whether they are alpha-convertible or not *)

let typecheck = ref false
(* takes as input a file containing a lambda expression and a type, separated by a colon (:), and ending with a period (.), and verifies that the typing relationship holds *)
...
let run () =
  try
    ...
    (if
       (flag_set && not is_lam)
       || ((not flag_set) && (not (extension = "")) && not is_8pus)
     then Printf.printf "Invalid file extension\n"
     else
       let lexbuf =
         Lexing.from_channel
       ...
       else Interactive.run_interactive lexbuf);
    flush stdout
  with e -> raise e
